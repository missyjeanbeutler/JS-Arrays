...
let divider = (arr) => {
  return [evenFinder(arr), arr.filter(e => {
    if(e % 2 !== 0) return e
  })]
}
...
let maker = () => {
  for(var i = 1, arr = []; i <= 215; i++) arr.push(i)
  return arr
}
...
devMountainEmployees.forEach((e, i) => {
  if(e.name === 'Cahlan') devMountainEmployees.splice(i, 1)
})
...
users.forEach((e, i) => {
  if(e.name === 'Tyler McGinnis') users.splice(i, 1)
})
